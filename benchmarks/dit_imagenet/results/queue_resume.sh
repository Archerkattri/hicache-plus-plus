...
#
# IMPORTANT — before running this, APPLY THE SIGN FIX (the post-ladder plan merged in):
#   cd ../..  &&  git apply /tmp/hicache_signfix/signfix.patch
#   (fixes hermite.py x=-k -> +k, tree.py hermite_coeff, adds regression tests)
#   ALSO port the same +k fix to bench_dit.py::taylor_forecast before the taylor cell.
#
...
CHECK
[ -f "$RES/fid10k/hermite_i4_fix.npz" ] || run hermite 4 "$RES/fid10k_fix"
[ -f "$RES/fid10k/hermite_i6_fix.npz" ] || run hermite 6 "$RES/fid10k_fix"
[ -f "$RES/fid10k/hermite_i8_fix.npz" ] || run hermite 8 "$RES/fid10k_fix"
[ -f "$RES/fid10k/auto_i4_fix.npz" ]    || run auto    4 "$RES/fid10k_fix"
